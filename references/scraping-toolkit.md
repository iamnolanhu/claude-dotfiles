...
| --- | --- | --- | --- | --- |
| **scrapegraph** | One-shot structured extraction, search+extract, multi-URL, free markdownify | OpenRouter key | ~¢ fractions/page (cheap LLM) | MCP `scrape.ftwr.vip` (default), local CLI `sgai`, skill `scrapegraph` |
...
| **crawl4ai** | Raw fetch → markdown/html, screenshot, PDF, JS exec; no LLM cost; fallback | bearer | free (self-hosted) | REST `crawl.ftwr.vip`, MCP `crawl4ai` |

## Examples

### scrapegraph — structured extraction (default)

**Default = the hosted MCP `scrape.ftwr.vip`.** When the `scrapegraph` MCP is connected,
...
curl -s -X POST "https://crawl.ftwr.vip/crawl" \
...
Remote MCPs (`scrape.ftwr.vip`, `parse`, `crawl4ai`) are wired via the `mcp-remote` stdio
...
- Hosted deploy: behind the ftwr.vip Traefik host (Docker; host/path details kept local, not in this public ref)
